...
export async function GET(request: NextRequest) {
    try {
        ...
        // One chart-range fetch + in-memory buckets (same totals as 4×N aggregates)
        const chartBuckets = getFinancialChartBuckets(resolved);
        const chartRange = getFinancialChartRange(chartBuckets);
        const [chartOfficeRows, chartPersonalRows] = await Promise.all([
            prisma.officeExpense.findMany({
                where: {
                    expenseDate: { gte: chartRange.startDate, lte: chartRange.endDate },
                    ...regionScope,
                },
                select: { type: true, amount: true, expenseDate: true },
            }),
        ...
        for (const row of chartOfficeRows) {
            const idx = findChartBucketIndex(chartBuckets, row.expenseDate);
            if (idx < 0) continue;
            const amount = Number(row.amount || 0);
            if (row.type === 'DAILY') chartData[idx].daily += amount;
            else if (row.type === 'RENT') chartData[idx].rent += amount;
            else if (row.type === 'VEHICLE') chartData[idx].vehicle += amount;
        }
